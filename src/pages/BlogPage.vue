<template>
  <div class="container fluid">
    ...
    <div class="row my-2">
      <div class="col">
        <comment v-for="comment in blog.comments" :key="comment.id" :comment="comment" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    // this.$store.dispatch("createCommentEntry", this.$route.params.id);
  },
  computed: {
    profile() {
  ...
  methods: {
    ...
    createCommentEntry(data) {
      this.$store.dispatch("createCommentEntry", {
        ...this.newComments,
